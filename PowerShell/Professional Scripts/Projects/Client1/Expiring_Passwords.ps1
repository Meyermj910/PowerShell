...
#This section is to grab the password "Settings"
##############################################################################
...
$Report = Get-ADUser -Server $Server -Filter {Enabled -eq $True -and PasswordNeverExpires -eq $False} –Properties Name, SamAccountName, EmailAddress, PasswordLastSet, Manager |
...
#Email to Help Desk/Manager/Director
$DeskOptions = @{
    'SmtpServer' = "smtprelay.domain.com"
    'To' = "Services@domain.com"
...
ForEach($_ in $Report){
 ...
 $UsersEmail = If($_.EmailAddress -eq $Null){
    then{Get-ADUser -Server $Server -Identity $_.SamAccountName -Properties extensionAttribute1 | Select-Object -ExpandProperty extensionAttribute1}
...
If you have any questions or concerns please email the Service Desk via support@a360inc.com or call (777)888-9999.
